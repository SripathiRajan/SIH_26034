...
LLM synthesis via Groq (primary) or Google GenAI (Gemini), with statutory fallback.
"""
...
class ChatResponse(BaseModel):
    ...
    llm_generated: Optional[bool] = None  # True if synthesized by Groq/Gemini, False if template/fallback
    llm_provider: Optional[str] = None  # "groq" | "gemini" | None — active provider for this deployment
    groq_configured: Optional[bool] = None  # True if GROQ_API_KEY is present; never exposes key value
...
def chat_status():
    ...
    SECURITY: Detects whether GROQ_API_KEY/GEMINI_API_KEY are configured,
    but never prints or returns their values.
    """
    return {
        "status": "active",
        ...
        "llm_model": (
            settings.GROQ_MODEL if settings.is_groq_configured
            else settings.GEMINI_MODEL if settings.is_gemini_configured
            else None
        ),
    }
