...
_CORPUS_CANDIDATES = [
    ...
    # One level up (when the repo root is nested one level deeper)
    Path(__file__).resolve().parent.parent.parent.parent / "backend" / "data" / "rules_corpus.json",
    # Relative to the current working directory (uvicorn launched from repo root)
    Path("backend") / "data" / "rules_corpus.json",
]
