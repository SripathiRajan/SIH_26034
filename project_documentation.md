...
## 4. Statutory RAG Chatbot (Ask Assistant)

A conversational assistant for enforcement officers and packagers, grounded in the official gazette corpus:

* **Knowledge Base:** `backend/data/rules_corpus.json` (chunked gazette notifications with G.S.R. numbers, amendment references, and effective dates, generated by `scripts/ingest_rules_pdfs.py`) is seeded into a FAISS vector store with multilingual MiniLM embeddings (Hindi + English) at server startup, plus 4 curated statutory FAQ entries.
* **Retrieval:** Top-3 passage search per query with gazette provenance returned as citations (`"G.S.R. 779(E) -- PCR_Amendment_...pdf"`).
* **Synthesis Provider Order:** Groq (`GROQ_API_KEY`, OpenAI-compatible endpoint, `openai/gpt-oss-120b`) → Gemini (`GEMINI_API_KEY`, `google-genai` SDK) → statutory template fallback (offline, zero external calls).
* **Honesty Flags:** Every response carries `llm_generated` and `llm_provider`; `/api/chat/status` exposes boolean `groq_configured`/`gemini_configured` diagnostics. API keys are never logged, printed, or returned.
* **API:** `POST /api/chat` (`{message}` → `{reply, citations, sources, llm_generated, llm_provider, corpus_chunks_loaded}`) and `GET /api/chat/status`.
* **Frontend:** `ChatScreen` calls the backend RAG endpoint (30 s timeout), renders statutory citations as chips, and falls back to a built-in keyword statutory engine when the backend is unreachable.

---

## 5. Multi-Angle Live Camera Capture Sessions

Field inspectors capture up to 6 package faces (front, back, MRP panel, flaps) in a single inspection session:

* **Session Lifecycle:** `POST /api/scan/session` (create/resume, 1–6 images, magic-byte + 10 MB validation) → live merged statutory coverage → `POST /api/scan/session/{id}/finalize` (exactly one persisted `ScanRecord` with `facesScanned` + `imageUris`) or `DELETE /api/scan/session/{id}` (idempotent discard with temp-file cleanup).
* **Session Store:** In-memory, thread-safe, 15-minute TTL, 20-session LRU eviction with automatic temp-file cleanup.
* **Quality Gates:** Per-view Laplacian sharpness and glare-ratio checks with operator hints.
* **Merging:** `api/compliance_merger.py::merge_package_faces` fuses fields across views; mandatory-field coverage (`found`/`missing`/`hintLine`) is computed live against the 8 statutory declarations.
* **Sequential OCR:** Views are processed strictly sequentially through the ensemble pipeline to avoid native OCR races on Windows.
* **Frontend:** `CaptureScreen` provides gallery multi-pick, native camera capture, a live viewfinder (Web: `getUserMedia` + canvas frame grab; native: `expo-camera` `CameraView` + `takePictureAsync`), a capture cart, and a live declaration coverage grid. Clearing the cart or removing thumbnails discards the backend session to prevent orphaned temp files.

---

## 6. Verification & Quality Assurance

* **Backend Test Suite:** Unit & integration tests passing (`tests/test_api_endpoints.py`, `tests/test_dates.py`, `tests/test_ocr_ensemble.py`, `tests/test_pipeline_extraction_rules.py`, `tests/test_rules.py`, `tests/test_scan_session.py`, `tests/test_gazette_ingestion.py`, `tests/test_rag_corpus.py`, `tests/test_gemini_backend_config.py`, `tests/test_groq_backend_config.py`). LLM provider keys are isolated in tests via `conftest.py` so the suite is fully hermetic.
